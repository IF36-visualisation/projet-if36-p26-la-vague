...
# 1. Extraction du code pays depuis playerLocation (partie avant le "/")
speedrun <- speedrun %>%
  mutate(
    countryCode = str_to_upper(str_extract(playerLocation, "^[^/]+"))
  )

# 2. Capitales nationales et leurs coordonnées (Natural Earth)
capitales <- ne_download(
  scale = 110, type = "populated_places", category = "cultural",
  returnclass = "sf"
) %>%
  filter(FEATURECLA == "Admin-0 capital") %>%        # uniquement les capitales
  ...
  st_drop_geometry() %>%
  group_by(countryCode) %>% slice(1) %>% ungroup()    # 1 ligne par pays (évite les doublons)

# 3. Jointure des coordonnées
speedrun <- speedrun %>%
  left_join(capitales, by = "countryCode")

# 4. Nom du pays en français + région (continent)
speedrun <- speedrun %>%
  mutate(
...
mediane_temps <- median(speedrun_temps$realTime)   # en secondes
...
```{r echo=FALSE}
speedrun %>% select(contains("lon"))
speedrun %>% select(contains("lat"))
runs_geo <- speedrun %>%
...
carte_monde <- ggplot() +
  annotation_borders("world", fill = "grey95", colour = "grey85") +
  geom_point(
    data = runs_geo %>% arrange(desc(nb_runs)),
    aes(x = lon, y = lat, size = nb_runs, colour = nb_runs),
    alpha = 0.7
  ) +
  geom_text_repel(
    data = top_pays,
    ...
    size          = 3,
    arrow         = arrow(length = unit(0.012, "npc")),  # la flèche
  ...
  scale_size_continuous(range = c(0.5, 6), trans = "log10") +
  coord_quickmap() +
  labs(
    title    = "Nombre de runs par pays (échelle log)",
    x = NULL, y = NULL,
    colour = "Nb de runs", size = "Nb de runs"
  ) +
  theme_void()
...
<<<<<<< HEAD
\newpage
## Est ce que genre les plus vendues sotn celles qu'on speedrun le plus? 
...
\newpage
## Est ce que genre les plateformes vendues sotn celles qu'on speedrun le plus?
...
\newpage
## 18. Est ce que genre les plateformes vendues sotn celles qu'on speedrun le plus?
...
\newpage
## 19. Quels jeux ont la plus forte communauté speedrun par rapport à leur nombre de joueurs ?
