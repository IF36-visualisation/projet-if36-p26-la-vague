...
Il est important de noter que le fichier date du **22 décembre 2016**, et ne contient donc pas de données récentes.
...
Dans l'optique de concevoir et de commercialiser le jeu parfait, nous débutons par une étude du marché : quelles plateformes ont généré le plus de ventes à l'échelle mondiale ? Identifier les machines dominantes nous indiquera sur quels supports viser pour maximiser l'audience potentielle de notre jeu. Nous supposons que le marché est historiquement dominé par quelques consoles phares de constructeurs majeurs (Sony, Nintendo), mais nous nous demandons si cette domination se concentre sur une ou deux machines, ou si elle se répartit entre les différentes générations.

La figure suivante est une capture issue du dashboard Shiny développé dans le cadre du projet. Elle n’a donc pas été générée directement dans le rapport.

```{r echo=FALSE, fig.cap="Capture de l'application Shiny : plateformes ayant généré le plus de ventes mondiales", out.width="90%"}
knitr::include_graphics("../images/vente_plateforme_GLOBAL.png")
```
...
## 2. Quelles sont les plateformes qui ont généré le plus de ventes par région ?
...
L’objectif ici est d’identifier les **genres** qui génèrent le plus de ventes à l’échelle mondiale. Nous pouvons nous attendre à ce que certains genres plus "populaires", comme les jeux d’action ou de sport, dominent le marché.
...
```{r echo=FALSE, fig.cap="Capture de l'application Shiny : ventes globales par genre de jeu vidéo", out.width="90%"}
knitr::include_graphics("../images/vente_genre_GLOBAL.png")
```
...
## 4. Quelle région contribue le plus aux ventes de jeux vidéo ? {#q4-regions}
...
En effet, ces analyses nous ont permis de comprendre *où* et *comment* se réalisent les ventes. Nous cherchons désormais à identifier **quels jeux concentrent réellement ce succès** à l’échelle mondiale. Pour cela, nous avons agrégé les ventes par **nom de jeu**, afin d’éviter les doublons liés aux différentes plateformes, puis extrait le **top 10 des jeux les plus vendus dans le monde**.

La figure suivante est une capture issue du dashboard Shiny développé dans le cadre du projet. Elle n’a donc pas été générée directement dans le rapport.

```{r echo=FALSE, fig.cap="Capture de l'application Shiny : top 10 des jeux vidéo les plus vendus", out.width="90%"}
knitr::include_graphics("../images/vente_game_GLOBAL.png")
```
...
ggplot(sales_long, aes(x = Sales, y = reorder(Genre, Sales), fill = Region)) +
  ...
  theme(
    axis.text.x = element_text(margin = margin(r = 5), size = 6, angle = 20, hjust = 1)
  )
```
...
Cette première partie nous a permis de mieux comprendre le marché dans lequel notre jeu devra se lancer. Plusieurs idées importantes en ressortent. Le marché est largement dominé par deux constructeurs : Sony et Nintendo. Leurs consoles, très répandues, concentrent la plupart des ventes. Sortir notre jeu sur une de ces plateformes est donc un bon moyen de toucher un large public.

Surtout, nous avons vu que le marché se divise en deux mondes différents. D'un côté, l'Occident (Amérique du Nord et Europe) forme un ensemble assez proche, qui aime les jeux d'action et de sport et achète surtout de grandes franchises connues partout. De l'autre, le Japon fonctionne presque en vase clos : il préfère ses propres licences et les jeux de rôle, ce qui le rend très difficile à conquérir pour un nouveau jeu. Il n'existe donc ni plateforme ni genre qui plaisent à tout le monde.

Tout cela nous pousse vers une stratégie claire : viser surtout les marchés occidentaux, qui sont les plus faciles à atteindre. Pour leur plaire, notre jeu a tout intérêt à s'orienter vers les genres qui dominent ces marchés, à savoir l'action, le sport ou le shooter. Sortir le jeu sur plusieurs plateformes peut aussi aider à le faire connaître, mais ne suffit pas à garantir son succès.

Nous avons enfin remarqué que le PC occupe une place étonnamment forte en Europe. Cette observation nous servira plus tard, lorsque nous croiserons ces résultats avec ceux de la seconde partie. Pour l'instant, nous laissons de côté la question commerciale et nous tournons vers un tout autre angle : l'activité de la communauté speedrun.
...
Notre objectif est de savoir quels jeux sont les plus speedrunnés afin d'avoir une idée du style de jeux que la communauté apprécie relancer.
...
À la lecture de ce graphique, le platformer semble être le genre de jeu le plus relancé par les joueurs, loin devant les jeux de course et les FPS. En effet, les jeux de plateforme se prêtent bien, comme nous l'avons vu précédemment, aux mécaniques de rejouabilité. Si nous voulons que notre jeu se classe parmi les plus speedrunnés, il peut être intéressant qu'il mélange les styles. Nous pouvons imaginer un platformer sous forme de course.
...
# On affiche seulement les labels des jeux peu vendus mais très speedrunnés
label_games <- quadrant_data %>%
  filter(quadrant == "Peu vendus mais très speedrunnés") %>%
  arrange(desc(nb_runs)) %>%
  slice_head(n = 10)

# Palette personnalisée
palette_quadrants <- c(
  "Faibles ventes + faible activité" = "#BDBDBD",
  "Peu vendus mais très speedrunnés" = "#F28E2B",
  "Très vendus et très speedrunnés" = "#2CBCC3",
  "Très vendus mais peu speedrunnés" = "#B07CC6"
)
...
## 17. Est ce que genre les plus vendues sotn celles qu'on speedrun le plus?
...
## 18. Est ce que les plateformes vendues sotn celles qu'on speedrun le plus?
...
## 19. Quels jeux possèdent le meilleur ratio “runs / ventes” ?
...
## 20. Les jeux les mieux notés attirent-ils davantage les speedrunners ?
...
## 21. Existe-t-il une “recette parfaite” du jeu speedrunnable ?
...
Les résultats mettent surtout en évidence plusieurs caractéristiques communes aux jeux les plus speedrunnés :

- un gameplay dynamique et technique ;
- une forte rejouabilité ;
- des mécaniques permettant l’optimisation et la créativité ;
- plusieurs catégories ou stratégies de speedrun ;
- une communauté engagée sur le long terme.
...
Au terme de cette enquête, nos trois axes d'analyse convergent vers un même objectif : définir le profil du jeu idéal, à la fois rentable commercialement et capable de fédérer une communauté de speedrun.

L'**analyse des ventes** a dessiné le marché : il est dominé par quelques plateformes à très large parc installé (Sony, Nintendo), porté par les genres Action, Sports et Shooter, et structuré en deux mondes - un Occident homogène (Amérique du Nord et Europe) friand d'action et de compétition, face à un Japon refermé sur ses RPG et ses franchises locales, quasi imprenable pour un nouveau venu.

L'**analyse de Speedrun.com** a révélé une autre logique : le platformer et la course règnent sur les leaderboards, le PC concentre l'essentiel des runs, la durée idéale se situe entre une demi-heure et une heure, et la communauté se concentre en Amérique du Nord et en Europe de l'Ouest anglophones, avec un net essor lié au streaming à partir de 2014.

Le **couplage des deux sources** a enfin montré que succès commercial et activité speedrun sont liés, mais faiblement : vendre beaucoup aide à la visibilité sans suffire. Ce qui distingue réellement les jeux les plus speedrunnés tient au game design - profondeur technique, rejouabilité, multiplicité des catégories d'optimisation et communauté engagée sur la durée.

De cette synthèse, nous tirons la carte d'identité du jeu à concevoir :

| Critère | Choix retenu |
|------------------------------------|------------------------------------|
| **Genre** | Platformer nerveux à dimension course/action : assez grand public pour vendre, assez technique pour s'optimiser |
| **Plateforme** | PC en priorité (reine du speedrun, forte en Europe), avec une sortie multiplateforme Sony/Xbox pour les ventes |
| **Marché cible** | Occident anglophone (Amérique du Nord + Europe de l'Ouest) ; localisation anglaise prioritaire ; Japon écarté |
| **Format d'une run** | 30 min à 1 h, niveaux courts et segmentés, propices aux tentatives répétées |
| **Mécaniques** | Forte rejouabilité, profondeur technique, plusieurs catégories de speedrun, chronomètre intégré et classements en ligne |
| **Univers** | Esthétique contemporaine, dans l'esprit des années 2010, pensée pour le streaming et le partage |
| **Communauté** | Animation sur le long terme : c'est le facteur le plus discriminant des jeux à forte activité speedrun |

Cette carte d'identité repose toutefois sur un compromis assumé. Les leviers du succès commercial (genre populaire, grand parc installé, licence internationale) ne sont pas exactement ceux du speedrun (game design, profondeur, rejouabilité) : notre jeu doit donc être assez accessible pour séduire le grand public occidental, tout en restant assez profond pour retenir les speedrunners. Le choix du platformer-course sur PC, ouvert au multiplateforme, vise précisément ce point d'équilibre.

Plusieurs limites invitent enfin à la prudence : le jeu de données de ventes s'arrête fin 2016 et ignore la décennie suivante, l'activité Speedrun.com sur-représente le PC et les communautés occidentales déjà installées, et de nombreuses métadonnées (genres, plateformes, localisation) restent lacunaires. Ces réserves n'invalident pas notre profil, mais rappellent qu'il décrit le jeu idéal *au regard de ces données*, et non une vérité absolue du marché.

## Ressentis personnels {.unlisted .unnumbered}

**Daria :** J’ai beaucoup apprécié travailler sur ce projet. J’ai appris à utiliser R, un langage très différent de ceux que j’avais abordés jusqu’à présent. Au-delà de cet apprentissage, la compétence principale que je retiens est l’utilisation de **Git**. Bien que j’en aie déjà entendu parler, je ne l’avais jamais utilisé en pratique. Malgré quelques merges ratées au début, j’ai beaucoup progressé et je suis convaincue que cette compétence me sera utile tout au long de ma carrière. En revanche, en raison de la répartition des tâches au sein du groupe, je n’ai pas eu l’occasion de travailler sur la partie **Shiny**, un aspect qu’il aurait été intéressant de découvrir.

**Nathan :** J’ai globalement apprécié ce projet, même si le choix des données ne correspondait pas, au départ, à un domaine dans lequel je souhaitais particulièrement me plonger. Il m’a permis de vivre une première véritable expérience de manipulation et d’analyse de données, tout en découvrant le langage R et son écosystème, j’ai également trouvé intéressant de développer un dashboard avec Shiny, une technologie différente des frameworks web que je connais. Avec plus de temps, j’aurais aimé approfondir la création de visualisations plus complexes, au-delà des graphiques simples comme les graphiques en barres, afin de proposer une analyse encore plus riche et plus poussée.

**Baptiste :** J'ai beaucoup apprécié ce projet, qui m'a permis de progresser sur R et plus particulièrement sur ggplot2 et la grammaire des graphiques. La compétence principale que je retiens est la capacité à construire un raisonnement complet autour d'un Dataset, où chaque graphique répond à une question et en soulève une nouvelle, plutôt que d'empiler des figures sans fil conducteur. Avec plus de temps, j'aurais aimé approfondir le couplage entre les deux jeux de données afin d'en tirer des conclusions plus robustes.

**Côme :** J'ai apprécié ce projet qui m'a permis de découvrir R et la logique de visualisation de données avec ggplot2. La compétence principale que je retiens est la capacité à structurer une analyse autour de questions précises et à choisir le bon type de graphique pour y répondre. L'expérience avec Power BI a également été enrichissante : j'ai pu constater à quel point l'outil est efficace pour explorer des données rapidement grâce au drag-and-drop et à l'interactivité, mais aussi ses limites dès que l'on souhaite personnaliser finement un visuel ou gérer des problèmes de typage de données. Avec plus de temps, j'aurais aimé appliquer les compétences acquises à une problématique différente, car le domaine du jeu vidéo n'était pas celui qui m'attirait le plus au départ — même si j'ai finalement trouvé l'analyse intéressante une fois plongé dans les données.

\newpage

# Annexes

## Annexe A - Correspondance des codes de plateformes {.unlisted .unnumbered}
...
Le tableau ci-dessous présente la répartition des principales tâches réalisées par chaque membre du groupe au cours du projet. Bien que certaines activités aient été effectuées collectivement (réunions, validation des résultats et préparation de la présentation orale), chaque membre a été responsable des éléments indiqués ci-dessous.

\newpage

## Annexe B - Travail effectué {.unlisted .unnumbered}

### Répartition des contributions

| Membre | Contributions |
|------------------|------------------------------------------------------|
| **Daria** | Questions 3, 5, 6 et 16 à 21 ; couplage des données. |
| **Nathan** | Rédaction du README ; questions 1 et 7 ; développement de l'application Shiny. |
| **Baptiste** | Questions 4 et 9 à 15 ; rédaction de la conclusion finale. |
| **Côme** | Question 2 ; réalisation du tableau de bord Power BI. |

### Travail réalisé en commun

Les tâches suivantes ont été réalisées collectivement par l'ensemble de l'équipe :

- Choix du thème et du jeu de données.
- Rédaction du compte rendu.
- Préparation et présentation de l'oral.
